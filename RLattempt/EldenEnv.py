...
import pytesseract
import directkeys
import keyboard                             # Pytesseract is not just a simple pip install.
import psutil
import os
...
class EldenEnv(gym.Env):
    ...
    def __init__(self, config):
        ...
        spaces_dict = {                                                                                                         #Observation space (img, prev_actions, state)
            'img': spaces.Box(low=0, high=255, shape=(286, 510, 4), dtype=np.uint8),                                  
        ...
        }#Image of the game #Last 10 actions as one hot encoded array #Stamina and helth of the player in percent
        ...
        self.observation_buffer = []                                                             #Reward of the previous step
        ...
        self.action = 0                                                      #The maximum reward that the agent has gotten in this fight
    ...
    def wait_for_loading_screen(self):
        ...
        while True: #We are forever takieng a screenshot and checking if it is a loading screen.
            ...
            if have_been_in_loading_screen and (time.time() - t_since_seen_next) > 2.5:             #We have been in a loading screen and left it for more than 2.5 seconds
                ...
                #break
            elif not have_been_in_loading_screen and ((time.time() - t_check_frozen_start) > 5):   #We have not entered a loading screen for 25 seconds. (return to bonfire and walk to boss) #⚔️ in pvp we use this for waiting for matchmaking
    ...
    def check_for_loading_screen(self, frame):
        ...
        ocr_config = ("-l eng --oem 1 --psm 7")
        pytesseract_output = pytesseract.image_to_string(next_text_image,  config=ocr_config) #reading text from the image cut out lang='eng',config='--psm 8 --oem 1'
    ...
    def step(self, action):
        ...
        self.reward, self.death, self.boss_death, self.duel_won = self.rewardGen.update(frame, self.first_step, self.action) #📍 2. Collect the reward based on the observation (reward of previous step)
        ...
        while(self.observation_counter<4):
            frame = self.cnn_input()
            frame = cv2.resize(frame, (510, 286))
    ...
    def reset(self):
        ...
        self.observation_counter = 0
        #observation = cv2.resize(frame, (MODEL_WIDTH, MODEL_HEIGHT))
        #observation = cv2.cvtColor(observation,cv2.COLOR_BGR2GRAY)    #Reset also returns the first observation for the agent
        spaces_dict = { 
